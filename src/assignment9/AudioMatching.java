...
//        System.out.println(result[2]);
        ...
        // Convert bin offset to seconds
        // Each bin has an offset of 512 samples for the beginning 
        // Sample rate of audio file is 44.1 khz
        int bin_offset1 = result[0];
        int bin_offset2 = result[1];
        int time_offset1 = 2 * (bin_offset1 * 512) / SAMPLERATE;
        int time_offset2 = 2 * (bin_offset2 * 512) / SAMPLERATE;

        if (result[2] >= binThreshold) {
            match = true;
            offset1 = time_offset1;
            offset2 = time_offset2;
        }
    }
