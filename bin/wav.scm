...
;
; wav 
; wav -summarize srcfile
; wav --summarize srcfile
; wav -copy srcfile dstfile
; wav -- --copy srcfile dstfile
; wav -extract time1 time2 srcfile dstfile
; wav --extract time1 time2 srcfile dstfile
;
; where
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; This Scheme script was obtained by modifying an R6RS top-level
; program.  See FIXME comments below.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
...
;;; FIXME
;;;
;;; For an R6RS top-level program, this line is:
;;;
;;; (define cmds (cdr (command-line)))
;;;
;;; For a Larceny Scheme script, the line has to be different.
;;; Scheme scripts were never standardized, and no attempt was
;;; made to specify the behavior of (command-line) in Scheme
;;; scripts.

(define cmds (cddr (command-line)))
...
;;; FIXME: The usage message has been modified because this
;;; is now a Larceny Scheme script.

(define (print-usage)
  (define msg0
    "Usage:")
  (define msg1
    (begin "larceny --r6rs --program wav.sps -- -copy srcfile dstfile"
           "wav -copy srcfile dstfile"))
  (define msg2
    (begin "larceny --r6rs --program wav.sps -- -summarize srcfile"
           "wav -summarize srcfile"))
  (define msg3
    (begin
...
(case operation
 ...
 (else
  #;(write operation)         ; FIXME: modified for Scheme script
  #;(newline)                 ; FIXME
  #;(write cmds)              ; FIXME
  #;(newline)                 ; FIXME
  (print-usage)))